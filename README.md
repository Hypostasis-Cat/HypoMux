...
HypoMux 不出售个人数据，也不默认上传遥测。程序会在用户或软件操作者请求相应功能时与其他网络系统通信：转发用户选择的网络流量、从官方 signed update channel 检查更新、从 GitHub 或 CNB Release 下载安装包，以及进行网络连通性验证。使用可选的内置 AI 助手时，对话及必要的工具结果会发送到用户配置的模型 API；启用外部 MCP 时，所开放的数据会返回连接的 AI 客户端。诊断默认脱敏，但应用名称、网卡标识、规则匹配值和用户手动输入仍可能包含个人信息。API 密钥与对话历史在本机加密保存，详见 [AI 助手与外部 MCP](docs/AI_ASSISTANT.md)。
...
<p align="center">
  <a href="https://github.com/Hypostasis-Cat/HypoMux"><img alt="海心，特别鸣谢 ¥100" src="https://img.shields.io/static/v1?label=%E6%B5%B7%E5%BF%83&message=%C2%A550&color=DCD0FF&labelColor=E6E6FA&style=for-the-badge&logo=github-sponsors&logoColor=6A5ACD" /></a>
