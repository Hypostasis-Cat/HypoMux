    ...
    {entries.length > 0 && <div className="tool-node-table" tabIndex={0} role="region" aria-label={en ? "Download nodes" : "下载节点"}>
      <table>
        ...
        <tbody>{entries.map(entry => <tr key={`${entry.adapter}/${entry.domain}/${entry.port}/${entry.ip}`}>
          ...
          <td className="steam-node-state"><strong>{Date.parse(entry.cooldown_until) > Date.now() ? (en ? "Cooling down" : "冷却中") : entry.preferred ? (en ? "Preferred" : "优先候选") : entry.validated ? (en ? "Verified candidate" : "验证通过的候选") : (en ? "Original node · observation only" : "原节点 · 仅观察")}</strong>
