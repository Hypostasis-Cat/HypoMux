...
export function AIAssistant({ open, onOpenChange, workspace = true, onOpenWorkspace, onStatusChange, page = "home" }: { open: boolean; onOpenChange: (open: boolean) => void; workspace?: boolean; page?: AppPage; onOpenWorkspace?: () => void; onStatusChange?: (status: { running: boolean; pending: number }) => void }) {
  ...
  const modelDirty = !!savedConfig && (config.protocol !== savedConfig.protocol || config.base_url !== savedConfig.base_url || config.model !== savedConfig.model || !!key || clearKey);
  ...
  const content = <>
      ...
      {view === "appearance" ? <Suspense fallback={<Spinner size="small" />}><SkinWardrobe /></Suspense> : view === "chat" ? <>
      ...
      </> : view === "model" ? <div className="ai-settings ai-deploy">
        <div className="ai-deploy-hero">
        ...
        <section className="ai-deploy-card" aria-labelledby="ai-endpoint-heading"><div className="ai-deploy-card-heading"><span className="ai-deploy-number">01</span><div><h3 id="ai-endpoint-heading">{text("连接模型服务", "Connect a model service")}</h3><p>{text("选择接口协议并填写服务地址。", "Choose an API protocol and enter its endpoint.")}</p></div></div>
          <div className="ai-deploy-field-grid"><Field label={text("接口协议", "API protocol")}><Select value={config.protocol} disabled={busy || snapshot.running} onChange={(_, d) => setConfig(c => ({ ...c, protocol: d.value }))}><option value="openai">OpenAI compatible · Chat Completions</option><option value="anthropic">Anthropic · Messages</option></Select></Field>
          <Field label="API Base URL" hint={text("包含版本前缀，如 https://api.example.com/v1；本地 HTTP 仅限回环地址。", "Include the version prefix, e.g. https://api.example.com/v1. HTTP is allowed only on loopback.")}><Input type="url" spellCheck={false} autoComplete="off" value={config.base_url} disabled={busy || snapshot.running} onChange={(_, d) => setConfig(c => ({ ...c, base_url: d.value }))} /></Field></div>
        </section>
        <section className="ai-deploy-card" aria-labelledby="ai-credentials-heading"><div className="ai-deploy-card-heading"><span className="ai-deploy-number">02</span><div><h3 id="ai-credentials-heading">{text("设置访问凭据", "Set access credentials")}</h3><p>{text("密钥保存在本机。无认证的本地服务可以留空。", "Your key is stored locally. Leave it blank for a local service without authentication.")}</p></div></div>
        ...
        <section className="ai-deploy-card" aria-labelledby="ai-model-heading"><div className="ai-deploy-card-heading"><span className="ai-deploy-number">03</span><div><h3 id="ai-model-heading">{text("选择可用模型", "Choose a model")}</h3><p>{text("可以从服务获取模型列表，也可以直接输入 Model ID。", "Fetch available models or enter a model ID directly.")}</p></div></div>
        ...
        <div className="ai-deploy-footer"><div><strong>{text("准备就绪后保存配置", "Save when you're ready")}</strong><small>{text("测试只检查连接与工具调用，不会修改网络。对话及必要的工具数据会发送至所选服务，可能产生费用；历史记录在本机加密保存，手动输入的内容不会自动脱敏。", "Testing checks the connection and tool calling without changing your network. Conversations and necessary tool data are sent to the selected service and may incur charges. History is encrypted locally; text you enter is not automatically redacted.")}</small></div><div className="ai-deploy-footer-actions"><Button disabled={busy || snapshot.running || !native} onClick={() => void action(async () => { await save(); setNotice(text("模型配置已保存。", "Model configuration saved.")); })}>{text("保存", "Save")}</Button><Button appearance="primary" disabled={busy || snapshot.running || !native} onClick={() => void action(async () => { await save(); setNotice(await aiService.test()); })}>{text("保存并测试工具调用", "Save and test tool calling")}</Button></div></div>
      </div> : <div className="ai-settings ai-deploy ai-deploy-external"><div className="ai-deploy-hero"><div className="ai-deploy-hero-copy"><span className="ai-deploy-eyebrow">HYPOMUX / MCP</span><h2>{text("连接外部 AI", "Connect an external AI")}</h2><p>{text("通过本机 MCP 接口，让支持 Streamable HTTP 的 AI 客户端使用 HypoMux。", "Let an AI client supporting Streamable HTTP use HypoMux through a local MCP endpoint.")}</p></div><div className="ai-deploy-status" data-state={mcp.enabled ? "saved" : "empty"}><span className="ai-deploy-status-dot" /><span><strong>{mcp.enabled ? text("本机连接已开启", "Local access enabled") : text("本机连接未开启", "Local access disabled")}</strong><small>{mcp.enabled ? (mcp.read_only ? text("只读模式", "Read-only mode") : text("操作需确认", "Changes need approval")) : text("关闭应用后连接失效", "Available while the app is open")}</small></span></div></div>
        <section className="ai-deploy-card" aria-labelledby="ai-mcp-access-heading"><div className="ai-deploy-card-heading"><span className="ai-deploy-number">01</span><div><h3 id="ai-mcp-access-heading">{text("设置本机访问", "Set local access")}</h3><p>{text("接口只监听本机，修改操作仍需在这里确认。", "The endpoint listens only on this device; changes still need approval here.")}</p></div></div><div className="ai-deploy-mcp-options"><Field label={text("本机端口", "Local port")}><Input type="number" min={1024} max={65535} value={port} disabled={mcp.enabled || busy} onChange={(_, d) => setPort(d.value)} /></Field><Checkbox label={text("只开放查询与体检", "Read-only queries and diagnostics")} checked={readOnly} disabled={mcp.enabled || busy} onChange={(_, d) => setReadOnly(d.checked === true)} /></div><Button appearance="primary" disabled={busy || !native} onClick={() => void action(async () => { if (mcp.enabled) { await aiService.disableMCP(); setMcp({ enabled: false, url: "", read_only: true }); setConnection(null); } else { const c = await aiService.enableMCP(Number(port), readOnly); setMcp(c.status); setConnection(c); } })}>{mcp.enabled ? text("关闭外部连接", "Disable external access") : text("启用本机连接", "Enable local access")}</Button></section>
        {mcp.enabled && <section className="ai-deploy-card" aria-labelledby="ai-mcp-connection-heading"><div className="ai-deploy-card-heading"><span className="ai-deploy-number">02</span><div><h3 id="ai-mcp-connection-heading">{text("添加到 AI 客户端", "Add to your AI client")}</h3><p>{text("将以下连接信息填写到支持自定义 MCP 的客户端。", "Add these details to a client that supports custom MCP connections.")}</p></div></div><Field label="MCP URL"><Input readOnly value={mcp.url} /></Field>{connection ? <><Field label={text("连接配置（含访问密钥，请勿发给模型）", "Connection configuration (contains a secret; do not send to a model)")}><Textarea readOnly value={JSON.stringify({ mcpServers: { hypomux: { type: "http", url: mcp.url, headers: { Authorization: `Bearer ${connection.token}` } } } }, null, 2)} resize="vertical" /></Field><Button disabled={busy} onClick={() => void action(async () => { await navigator.clipboard.writeText(JSON.stringify({ mcpServers: { hypomux: { type: "http", url: mcp.url, headers: { Authorization: `Bearer ${connection.token}` } } } }, null, 2)); setNotice(text("已复制。请粘贴到客户端连接设置；不同客户端配置格式可能不同。", "Copied. Paste into client connection settings; configuration formats vary.")); })}>{text("复制连接配置", "Copy connection configuration")}</Button></> : <p>{text("访问密钥仅在启用时显示。重新关闭并启用可生成新密钥，旧连接将失效。", "The secret is shown only on activation. Disable and enable to rotate it.")}</p>}</section>}
