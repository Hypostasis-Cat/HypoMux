...
export function AssistantCompanion({ open, onOpenChange, running, pending, pageLabel, speech, speechId, sample = false, hidden = false, onViewDetails, onSpeechDismiss, children }: {
  ...
  const speechWidth = Math.max(0, Math.min(300, (leftSide ? right : viewport.width - right - petWidth) - 28));
  // Both short hints and replies attach outside the character bounds. A skin's
  // internal anchor must not pull a wide reply over the character.
  const sidePosition = leftSide
    ? { left: viewport.width - right + 12, right: "auto" }
    : { left: "auto", right: right + petWidth + 12 };
