...
<feed xmlns="http://www.w3.org/2005/Atom">
  <entry>
    <title>HypoMux 2.5.7</title>
    <link rel="alternate" href="https://github.com/Hypostasis-Cat/HypoMux/releases/tag/v2.5.7" />
  </entry>
</feed>`
	client := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		case request.Method == http.MethodHead && request.URL.String() ==
			releaseDownloadURL+"v2.5.7/HypoMux_Setup_2.5.7.exe":
			response.StatusCode = http.StatusOK
	...
	if !result.Available || result.Release.TagName != "v2.5.7" {
		t.Fatalf("fallback update result = %#v", result)
	}
	if result.Release.InstallerName != "HypoMux_Setup_2.5.7.exe" ||
