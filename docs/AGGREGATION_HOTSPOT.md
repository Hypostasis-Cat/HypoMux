...
已存在的热点/ICS 会阻止启动，避免覆盖其他共享。工作进程每三秒检查 TUN、热点网卡与共享出口；明确发现出口不匹配、TUN 消失、本地网关失效或父进程管道关闭时停止热点。传统 ICS 从有记录变为空记录时只撤销“出口已验证”标记。正常停止聚合时先关闭热点，再停止 TUN。退出和启动失败时尝试恢复原热点配置，清理失败会显示错误。密码仅通过标准输入传给工作进程，不放入命令行、HypoMux 设置或日志；Windows 在运行期间会持久化热点配置，正常关闭后恢复原配置。工作进程被强制结束或系统断电时不能保证恢复完成，应在 Windows 设置中检查。
...
- 尚未完成实际手机端到端验收：包括转发进入 sing-box、ICS DNS 与严格路由兼容性、IPv6、UDP/QUIC、MTU、频段与无线驱动组合。该功能应保持实验性标记。
