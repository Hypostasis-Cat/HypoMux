...
3. 优先获取当前提升令牌对应的 UAC linked token，并使用同账户的普通完整性令牌创建暂停态替代 UI。
4. linked token 路径不可用时，尝试交互式 Windows Shell 令牌；若令牌创建 API 仍受系统权限策略阻止，则仿照 PowerToys 把 Explorer 指定为逻辑父进程，通过其普通令牌创建替代 UI。该后备路径同时避免继承旧计划任务的 Job。
5. 每条路径都必须验证替代 UI 确实未提升且用户 SID 正确，验证通过后才恢复其线程并退出短生命周期的管理员入口进程。
6. 替代进程携带内部重启标记；若系统策略再次把它提升，则停止自动重试以避免循环。
7. 若所有降权路径均失败，进入显式管理员兼容模式：UI 权限只生成安全提醒，不作为系统代理或 TUN 的功能门禁；TUN、WFP、路由与恢复仍通过独立 Core Service 或经身份验证的按需 Core 执行。
8. 若无法验证管理员身份与交互式桌面用户相同，TUN 仍可通过独立核心使用；系统代理继续保护当前用户的 `HKCU` 边界，避免修改错误账户。

安装、升级和卸载还会无条件尝试删除旧计划任务，使升级后的下一次登录不再重复进入管理员入口。Core Service 接受活动控制台会话中的普通或提升 UI 客户端；命名管道 ACL、本机传输、具体客户端 PID 与活动会话校验继续生效。
